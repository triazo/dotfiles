...
# Use hyprctl to get list of monitors
...
while true; do
    ...
    current_monitor=$(echo "$monitors" | cut -d',' -f$((idm + 1)))
    wallpaper_cmd="hyprctl hyprpaper wallpaper $current_monitor,\"$current_wallpaper\""
    eval "$wallpaper_cmd"
    idx=$(((idx + 1) % n))
    idm=$(((idm + 1) % n))
done
